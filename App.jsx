...
{week:3,type:"challenge",p1:"Gaurav Kukreja",p2:"Saied Delagah",winner:"Gaurav Kukreja",score:"6-0, 6-1"},
// ── WEEK 4 ──
{week:4,type:"assigned",p1:"Peter Champe",p2:"Jason Eckstein",winner:"Jason Eckstein",score:"6-1, 6-2"},
{week:4,type:"assigned",p1:"Victor Casler",p2:"Phil Astras",winner:"Victor Casler",score:"6-0, 6-0"},
{week:4,type:"assigned",p1:"Lorne Noble",p2:"Ming Cai",winner:"Lorne Noble",score:"6-3, 2-6, 1-0"},
{week:4,type:"assigned",p1:"JD Tulloch",p2:"Dez Nagle",winner:"JD Tulloch",score:"6-4, 6-3"},
{week:4,type:"assigned",p1:"Jordan Logan",p2:"Dave Corujo",winner:"Jordan Logan",score:"6-2, 6-2"},
{week:4,type:"assigned",p1:"Brent Nelson",p2:"James Atwell",winner:"Brent Nelson",score:"6-2, 6-3"},
{week:4,type:"assigned",p1:"Ricardo Yunis",p2:"Aaron Pattillo",winner:"Aaron Pattillo",score:"6-4, 3-6, 7-6"},
{week:4,type:"assigned",p1:"Ryan Draves",p2:"Patrick Elliott",winner:"Patrick Elliott",score:"2-6, 6-2, 6-3"},
{week:4,type:"assigned",p1:"Michael Voecks",p2:"Aaron Schacht",winner:"Aaron Schacht",score:"6-0, 6-4"},
...
MATCHES.filter(m => m.week <= w && m.type === "assigned").forEach(m => {
...
return { week: w, label: `Week ${w}`, rank: rank > 0 ? rank : null, points: pts[playerName] || 0 };
}).filter(r => r.rank !== null);
}
...
m.week === week &&
(m.p1 === playerName || m.p2 === playerName) &&
(matchTypeFilter === "all" ? true : m.type === matchTypeFilter)
);
let pts = 0;
matches.forEach(m => {
const { winnerPts, loserPts } = computeMatchPoints(m);
pts += m.winner === playerName ? winnerPts : loserPts;
});
return pts;
}
...
const hist = getRankHistoryAssigned(playerName);
...
const [standingsMode, setStandingsMode] = useState("assigned");
...
const { winnerPts, loserPts } = computeMatchPoints(m);
const loser = m.winner === m.p1 ? m.p2 : m.p1;
assignedPts[m.winner] = (assignedPts[m.winner] || 0) + winnerPts;
assignedPts[loser] = (assignedPts[loser] || 0) + loserPts;
});
const sortedAssigned = Object.entries(assignedPts).sort(([,a],[,b]) => b - a || 0);
const assignedRank = sortedAssigned.findIndex(([name]) => name === selectedPlayer) + 1;
const assignedPoints = assignedPts[selectedPlayer] || 0;
return {
name: selectedPlayer,
...
currentRank: assignedRank || null,
currentPoints: assignedPoints,
};
})() : null;

const directoryList = useMemo(() => {
const list = WEEK_STANDINGS[CURRENT_WEEK - 1].standings.map(s => ({
...
<p style={{margin:"4px 0 0",fontSize:15,color:"#1c1917"}}>
...
return (
<p style={{margin:"2px 0 0",fontSize:13,color:"#1c1917"}}>
...
<p style={{margin:"4px 0 0",fontSize:13,color:"#1c1917"}}>
...
const { winnerPts, loserPts } = computeMatchPoints(m);
const ptsEarned = won ? winnerPts : loserPts;
return (
<div key={i} style={{display:"grid",gridTemplateColumns:"1fr auto auto",alignItems:"center",gap:8,padding:"8px 12px",borderRadius:8,fontSize:13,
...
padding:"1px 5px",borderRadius:4}}>{m.type === "challenge" ? "CH" : "ASN"}</span>
</div>
<span style={{fontWeight:700,color:won?"#16a34a":"#78716c",fontSize:12,whiteSpace:"nowrap"}}>{ptsEarned} pts</span>
<span style={{color:"#1c1917",fontSize:11,fontFamily:"'SF Mono',monospace",whiteSpace:"nowrap"}}>Wk{m.week} &middot; {m.score}</span>
</div>
);
})}
</div>
...
let list = Object.entries(pts)
.sort(([,a],[,b]) => b - a || 0)
.map(([name, points], i) => {
const rec = getRecordByTypeThruWeek(name, "assigned", selectedWeek);
return { name, points, rank: i + 1, wins: rec.wins, losses: rec.losses };
});
...
{standingsMode === "all" ? (
<>
<div style={{display:"grid",gridTemplateColumns:"42px 1fr 55px 50px 60px 90px",padding:"8px 12px",fontSize:11,fontWeight:600,color:"#a8a29e",borderBottom:"1px solid #e7e5e4",textTransform:"uppercase",letterSpacing:"0.05em"}}>
<span>#</span><span>Player</span><span>Total</span><span>+Wk</span><span>Move</span><span>Trend</span>
</div>
{displayList.map(s => {
...
borderBottom:"1px solid #f5f5f4",background:"transparent",transition:"background 0.1s"}}
...
<span style={{fontWeight:600,fontSize:12,color:wkPts>0?"#16a34a":"#a8a29e"}}>{wkPts>0?`+${wkPts}`:"—"}</span>
...
return (
<div key={s.name} onClick={() => setSelectedPlayer(s.name)}
style={{display:"grid",gridTemplateColumns:"42px 1fr 55px 50px 80px",padding:"10px 12px",fontSize:13,cursor:"pointer",alignItems:"center",
borderBottom:"1px solid #f5f5f4",background:"transparent",transition:"background 0.1s"}}
onMouseEnter={e => { e.currentTarget.style.background="#fafaf9"; }}
onMouseLeave={e => { e.currentTarget.style.background="transparent"; }}>
<span style={{fontWeight:700,color:s.rank<=3?"#1a472a":"#78716c",fontSize:14}}>{s.rank}</span>
...
<span style={{fontWeight:m.winner===m.p1?700:400,color:m.winner===m.p1?"#16a34a":"#1c1917"}}>{m.p1}</span>
<span style={{color:"#44403c",fontSize:11}}>vs</span>
<span style={{fontWeight:m.winner===m.p2?700:400,color:m.winner===m.p2?"#16a34a":"#1c1917"}}>{m.p2}</span>
</div>
<span style={{color:"#1c1917",fontSize:12,fontFamily:"'SF Mono',monospace"}}>{m.score}</span>
</div>
);
...
<h3 style={{margin:0,fontSize:15,fontWeight:700}}>Week {wk} Assigned Matches</h3>
...
<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
<h3 style={{margin:0,fontSize:15,fontWeight:700,color:"#92400e"}}>Week {wk} Challenge Matches</h3>
...
const allPlayers = WEEK_STANDINGS[CURRENT_WEEK - 1].standings;
